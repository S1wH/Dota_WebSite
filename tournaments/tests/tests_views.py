...
class TestTournamentView(TestCase):
    ...
    def test_current_list_content(self):
        mixer.blend(Tournament, start_date=timezone.now() - timedelta(days=1),
                    end_date=timezone.now() + timedelta(days=5))
        mixer.blend(Tournament, start_date=timezone.now() - timedelta(days=2),
                    end_date=timezone.now() + timedelta(days=10))
        mixer.blend(Tournament, start_date=timezone.now() - timedelta(days=3),
    ...
    def test_previous_list_content(self):
        mixer.blend(Tournament, start_date=timezone.now() - timedelta(days=10),
                    end_date=timezone.now() - timedelta(days=5))
        mixer.blend(Tournament, start_date=timezone.now() - timedelta(days=20),
                    end_date=timezone.now() - timedelta(days=10))
        mixer.blend(Tournament, start_date=timezone.now() - timedelta(days=30),
                    end_date=timezone.now() - timedelta(days=15))
        response = self.client.get('/tournaments/previous_tournaments/')
        self.assertEqual(len(response.context['object_list']), 3)
