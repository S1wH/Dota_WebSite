...
class TestMatchView(TestCase):
    ...
    def test_ongoing_list_content(self):
        ...
        mixer.blend(
            Match, start_date=timezone.now() - timedelta(minutes=10), end_date=None
        )
        mixer.blend(
            Match, start_date=timezone.now() - timedelta(minutes=15), end_date=None
        )
        response = self.client.get("/matches/ongoing_matches/")
        self.assertEqual(len(response.context["object_list"]), 2)

    def test_incoming_list_content(self):
        mixer.blend(Match, start_date=timezone.now() + timedelta(days=3), end_date=None)
        mixer.blend(
            Match, start_date=timezone.now() - timedelta(minutes=10), end_date=None
        )
        mixer.blend(
            Match, start_date=timezone.now() + timedelta(minutes=35), end_date=None
        )
        response = self.client.get("/matches/incoming_matches/")
        self.assertEqual(len(response.context["object_list"]), 2)
    ...
    def test_played_list_content(self):
        ...
        mixer.blend(
            Match, start_date=timezone.now() - timedelta(minutes=30), end_date=None
        )
        mixer.blend(
            Match,
