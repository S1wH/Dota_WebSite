...
class TestMatches(APITestCase):
    ...
    def setUp(self):
        self.url_v1 = '/matches/v1.0/api/matches/'
        self.url_v2 = '/matches/v1.1/api/matches/'
        self.auth_client = APIClient()
        user = mixer.blend(MyUser)
        self.auth_client.force_authenticate(user=user)

    def test_status_code_guest(self):
        response = self.client.get(self.url_v1)
    ...
    def test_status_code_auth(self):
        response = self.auth_client.get(self.url_v1)
        self.assertEqual(response.status_code, 200)

        response = self.auth_client.get(self.url_v2)
        self.assertEqual(response.status_code, 200)
...
class TestMatchPeriod(APITestCase):
    ...
    def setUp(self):
        self.url_v1 = '/matches/v1.0/api/match_periods/'
        self.url_v2 = '/matches/v1.1/api/match_periods/'
        self.auth_client = APIClient()
        user = mixer.blend(MyUser)
        self.auth_client.force_authenticate(user=user)
    ...
    def test_post(self):
        ...
        response = self.auth_client.post(self.url_v1,
                                         {
        ...
        response = self.auth_client.post(self.url_v2,
                                         {
                                             'duration': timedelta(minutes=45),
                                             'win_team': team.id,
                                             'match': match.id,
    ...
    def test_put(self):
        ...
        url = self.url_v1 + f'{match_period.id}/'
        match = mixer.blend(Match)
        team = mixer.blend(Team)
        ...
        url = self.url_v2 + f'{match_period.id}/'
        response = self.auth_client.put(url,
                                        {
                                            'duration': timedelta(minutes=35),
                                            'win_team': team.id,
